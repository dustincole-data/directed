...
export function render(svg, data) {
  ...
  function roundedBarPath(x, y, w, h, r, roundRight) {
    if (roundRight) {
  ...
  const barHeight = Math.min(bandHeight * 0.6, 24);

  const bg = el("rect", { x: 0, y: 0, width, height, fill: "#fcfcfb" });
  svg.appendChild(bg);

  const title = el(
    "text",
    { x: margin.left, y: 24, "font-size": "16", "font-weight": "bold", fill: "#0b0b0b" },
  ...
  const subtitle = el(
    "text",
    { x: margin.left, y: 40, "font-size": "11", fill: "#52514e" },
  ...
  rows.forEach((row, i) => {
    ...
    const color = isPositive ? "#2a78d6" : "#e34948";
    const r = Math.min(4, barWidth, barHeight / 2);

    const bar = el("path", {
      d: roundedBarPath(barX, barY, barWidth, barHeight, r, isPositive),
      fill: color,
    });
